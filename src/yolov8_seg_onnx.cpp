...
using namespace Ort;
#include <chrono>  // Tambahkan header ini untuk pencatatan waktu
...
bool Yolov8SegOnnx::OnnxBatchDetect(std::vector<cv::Mat>& srcImgs, std::vector<std::vector<OutputParams>>& output) {
    ...
    // Mulai pencatatan waktu inferensi
    ...
    // Akhir pencatatan waktu inferensi
    auto end_time = high_resolution_clock::now();
    auto duration = duration_cast<milliseconds>(end_time - start_time);
    
    // Hitung inference time per frame
    double inference_time_per_frame = duration.count() / static_cast<double>(srcImgs.size());

    std::cout << "Total Inference Time: " << duration.count() << " ms" << std::endl;
    std::cout << "Inference Time per Frame: " << inference_time_per_frame << " ms/frame" << std::endl;
    ...
    int64_t one_output_length = VectorProduct(_outputTensorShape) / _outputTensorShape[0];
    int net_width = (int)_outputTensorShape[1];
    int score_array_length = net_width - 4 - _outputMaskTensorShape[1];
    ...
    for (int img_index = 0; img_index < srcImgs.size(); ++img_index) {
        ...
        for (int r = 0; r < rows; ++r) {
            ...
            if (max_class_score >= _classThreshold) {
                vector<float> temp_proto(pdata + 4 + score_array_length, pdata + net_width);
                picked_proposals.push_back(temp_proto);
