...
int main(int argc, char **argv) {
    ...
    while (!SLAM.isShutDown()&& b_continue_session)
    {
        ...
        // rs2::video_frame color_frame = fs.first(RS2_STREAM_COLOR);
        ...
        float viewer_fps = SLAM.GetViewerFPS();
        log_file << std::fixed << std::setprecision(6) << timestamp << "\t" << viewer_fps << "\t" << t_track << "\n";

#endif
    }
    cout << "Saving keyframe trajectory..." << endl;
    SLAM.SaveTrajectoryTUM("/home/knowfix/CameraTrajectory.txt");
